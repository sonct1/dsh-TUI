...
| `Shift+Tab` | 循环当前模型支持的推理强度（按 adapter 提供的顺序） |
...
整屏场景（不污染 scrollback），查看会话全程的时间线。适配层提供完整常驻的 `traceEvents()` 快照，因此没有伪造的“更早历史”分页：顶部就是常驻日志的第一条事件；裸词全文检索覆盖全部可检查记录（系统/上下文提示、工具、前序提示和 diff、助手与工具输出、聚合成员、压缩）。若后端未来提供不完整前缀，必须先显式暴露边界，UI 才会标记历史不完整。
...
| `m` | 循环投影模式（等分 / 墙钟 / 压缩空闲） |
| `v` / `x` | 在光标处标记范围起止 / 清除范围；完成范围后账本聚焦该闭区间 |
| `z` | 将波形缩放到完成范围；再次按下恢复全域 |
| `c` / `Shift+C` | 折叠/展开当前请求/步骤；Shift+C 切换整个轮次 |
| `g` / `G` | 跳到顶部 / 底部 |
| `Enter` | 展开详情；`Tab` 切换可检查内容页，`n`/`p` 切换聚合成员，`j`/`k` 在详情内翻页 |
